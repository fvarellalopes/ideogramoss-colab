  ...
  def from_pretrained(
    ...
    *,
    ...
    conditional_transformer = _build_transformer(
      transformer_config, conditional_state_dict, load_device, dtype
    )
    del conditional_state_dict
    unconditional_transformer = _build_transformer(
      transformer_config, unconditional_state_dict, load_device, dtype
    )
    ...
    autoencoder = _load_autoencoder(autoencoder_weights, load_device, dtype)

    # Move to target device if colab_friendly
    if config.colab_friendly and load_device != device:
      print(f"Moving components to {device}...")
      conditional_transformer.to(device)
